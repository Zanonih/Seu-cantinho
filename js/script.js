...
// ---- Suculenta da relação: cresce conforme desejos marcados, filmes
// assistidos e receitas feitas (incluindo os sorteados na roleta) ----
const SUCULENTA_NIVEIS = [
  { min: 0, max: 7, nome: "Brotinho", gif: "img/suculenta/nivel1.gif" },
  { min: 8, max: 20, nome: "Crescendo", gif: "img/suculenta/nivel2.gif" },
  { min: 21, max: 40, nome: "Quase florida", gif: "img/suculenta/nivel3.gif" },
  { min: 41, max: Infinity, nome: "Florida", gif: "img/suculenta/nivel4.gif" },
];

function initSuculenta(config) {
  const {
    imagemId, nomeId, progressoId, legendaId, detalhesId, sedeId,
    desejos, filmes, receitas,
  } = config;

  const imagem = imagemId ? document.getElementById(imagemId) : null;
  const nomeEl = nomeId ? document.getElementById(nomeId) : null;
  const progressoEl = progressoId ? document.getElementById(progressoId) : null;
  const legendaEl = legendaId ? document.getElementById(legendaId) : null;
  const detalhesEl = detalhesId ? document.getElementById(detalhesId) : null;
  const sedeEl = sedeId ? document.getElementById(sedeId) : null;

  function contarSalvos(chave, itens) {
    try {
      const salvo = JSON.parse(localStorage.getItem(chave) || "[]");
      return Array.isArray(salvo) ? salvo.filter((v) => itens.includes(v)).length : 0;
    } catch {
      return 0;
    }
  }

  // ---- Regadas e solzinhos já dados (datas guardadas em cantinho:suculenta:regas / :sol) ----
  function carregarDatas(chave) {
    try {
      const salvo = JSON.parse(localStorage.getItem(chave) || "[]");
  ...
  const qtdDesejos = contarSalvos("cantinho:desejos:desejos", desejos || []);
  const qtdFilmes = contarSalvos("cantinho:sorteados:filmes", filmes || []);
  const qtdReceitas = contarSalvos("cantinho:sorteados:receitas", receitas || []);
  const datasRegas = carregarDatas("cantinho:suculenta:regas");
  const datasSol = carregarDatas("cantinho:suculenta:sol");
  const qtdRegas = datasRegas.length;
  const qtdSol = datasSol.length;
  const total = qtdDesejos + qtdFilmes + qtdReceitas + qtdRegas + qtdSol;

  // ---- Ela murcha um pouquinho (só visual, sem perder pontos) se
  // ninguém regar nem dar sol por uns dias ----
  const todasDatasCuidado = [...datasRegas, ...datasSol].sort();
  let estadoSede = null; // null | "leve" | "forte"
  if (todasDatasCuidado.length > 0) {
    const dias = diasDesde(todasDatasCuidado[todasDatasCuidado.length - 1]);
    if (dias >= 6) estadoSede = "forte";
    else if (dias >= 3) estadoSede = "leve";
  }

  const nivel = SUCULENTA_NIVEIS.find((n) => total >= n.min && total <= n.max) || SUCULENTA_NIVEIS[SUCULENTA_NIVEIS.length - 1];
  const indiceNivel = SUCULENTA_NIVEIS.indexOf(nivel);
  const ultimoNivel = indiceNivel === SUCULENTA_NIVEIS.length - 1;

  if (imagem) {
    imagem.src = nivel.gif;
    imagem.alt = `Nossa suculenta no estágio: ${nivel.nome}`;
    imagem.classList.toggle("is-sedenta-leve", estadoSede === "leve");
    imagem.classList.toggle("is-sedenta-forte", estadoSede === "forte");
  }

  if (nomeEl) nomeEl.textContent = nivel.nome;

  if (progressoEl) {
    const faixa = ultimoNivel ? 1 : (nivel.max - nivel.min + 1);
    const andamento = ultimoNivel ? 1 : Math.min(1, (total - nivel.min) / faixa);
    const pct = Math.round(andamento * 100);
    progressoEl.innerHTML = `
      <div class="conquistas-progresso-bar" style="height:12px;">
        <div class="conquistas-progresso-fill" style="width:${pct}%"></div>
      </div>`;
  }

  if (legendaEl) {
    legendaEl.textContent = ultimoNivel
      ? `${total} pontinhos ao todo — ela está no auge! 🌸`
      : `${total} pontinhos até agora · faltam ${nivel.max - total + 1} pro próximo estágio`;
  }

  if (sedeEl) {
    if (estadoSede === "forte") {
      sedeEl.textContent = "🥀 ela tá com bastante sede... regue ou dê um solzinho pra ela animar!";
    } else if (estadoSede === "leve") {
      sedeEl.textContent = "🥀 ela já tá pedindo água ou um solzinho fazem alguns dias...";
    } else {
      sedeEl.textContent = "";
    }
  }
  ...
  if (detalhesEl) {
    detalhesEl.innerHTML = `
      <li>✨ ${qtdDesejos} de ${(desejos || []).length} desejos realizados</li>
      <li>🎬 ${qtdFilmes} de ${(filmes || []).length} filmes/séries assistidos</li>
      <li>🍲 ${qtdReceitas} de ${(receitas || []).length} receitas feitas</li>
      <li>💧 regada ${qtdRegas} ${qtdRegas === 1 ? "vez" : "vezes"}</li>
      <li>☀️ tomou sol ${qtdSol} ${qtdSol === 1 ? "vez" : "vezes"}</li>
    `;
  }
}

// ---- Botões de regar e dar sol na suculenta (1 ponto cada, 1x por dia) ----
function initSuculentaCuidados(config) {
  const { regarBtnId, solBtnId, imagemId, aoAtualizar } = config;
  const btnRegar = regarBtnId ? document.getElementById(regarBtnId) : null;
  const btnSol = solBtnId ? document.getElementById(solBtnId) : null;
  const imagem = imagemId ? document.getElementById(imagemId) : null;
  ...
  function salvarDatas(chave, datas) {
    try {
      localStorage.setItem(chave, JSON.stringify(datas));
    } catch {
      // localStorage bloqueado — só não persiste
    }
  }

  function reagirFeliz() {
    if (!imagem) return;
    imagem.classList.remove("is-feliz");
    void imagem.offsetWidth; // reinicia a animação mesmo se já tiver rodado
    imagem.classList.add("is-feliz");
  }

  function configurarBotao(botao, chave, rotuloAtivo, rotuloFeito) {
    if (!botao) return;

    function atualizarEstado() {
      const datas = carregarDatas(chave);
      const jaFeitoHoje = datas.includes(hojeISO());
      botao.disabled = jaFeitoHoje;
      botao.textContent = jaFeitoHoje ? rotuloFeito : rotuloAtivo;
      botao.classList.toggle("is-feito", jaFeitoHoje);
    }
  ...
  configurarBotao(btnRegar, "cantinho:suculenta:regas", "💧 Regar", "💧 Regada hoje");
  configurarBotao(btnSol, "cantinho:suculenta:sol", "☀️ Dar sol", "☀️ Já tomou sol hoje");
}
